...
class Monopolar_VAE(nn.Module):
    ...
    def decode(self, amps, sampled_n_loc, ch_locs, exp_ids):
        ...
        #recon_amps = -torch.exp(distances*b_exps + a_exps) #original one
       
        alpha = torch.sum(distances * amps[:,:,0], dim=1) / (torch.sum(torch.square(distances*(amps[:,:,0]<0)), dim=1)+0.0001)
#         print(alpha.shape, torch.sum(torch.square(distances), dim=1).shape, torch.sum(distances * amps[:,:,0], dim=1).shape, torch.tile(alpha, (25,1)).T.shape, distances.shape)
#         print(distances)
      
        recon_amps = torch.tile(alpha, (25,1)).T / ((distances*(amps[:,:,0]<0))+0.0001)
        return recon_amps
